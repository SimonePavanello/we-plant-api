...
import cloud.nino.nino.service.custom.StopCustomService;
...
public class StopCustomResource {

    private final Logger log = LoggerFactory.getLogger(StopCustomResource.class);

    private static final String ENTITY_NAME = "stop";

    private final StopCustomService stopService;

    public StopCustomResource(StopCustomService stopService) {
    ...
    /**
     * GET  /stops/by-visit-id:visitId
     *
     * @param visitId the id of the visit to retrieve
     */
    @GetMapping("/stops/by-visit-id/{visitId}")
    @Timed
    public Collection<StopDTO> getStopsByVisitId(@PathVariable Long visitId) {
        log.debug("REST request to get Stops by visit id : {}", visitId);
        return stopService.findByVisitId(visitId);
    }
