...
import cloud.nino.nino.service.custom.AlberoVisitCustomService;
...
public class AlberoVisitCustomResource {

    private final Logger log = LoggerFactory.getLogger(AlberoVisitCustomResource.class);

    private static final String ENTITY_NAME = "alberoVisit";

    private final AlberoVisitCustomService alberoVisitCustomService;
    private final UserService userService;

    public AlberoVisitCustomResource(AlberoVisitCustomService alberoVisitService, UserService userService) {
        this.alberoVisitCustomService = alberoVisitService;
    ...
    public ResponseEntity<AlberoVisitDTO> createAlberoVisit(@RequestBody AlberoVisitDTO alberoVisitDTO) throws URISyntaxException {
        ...
        alberoVisitDTO.setVisitTime(ZonedDateTime.now());
        alberoVisitDTO.setUserId(userService.getCurrentUser().get().getId());
        AlberoVisitDTO result = alberoVisitCustomService.save(alberoVisitDTO);
    ...
    public List<AlberoVisitDTO> getAllAlberoVisits() {
        ...
        return alberoVisitCustomService.findAll();
    }
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of alberoVisits in body
     */
    @GetMapping("/albero-visits/by-autogenerated-users")
    @Timed
    public List<AlberoVisitEssenza> getAllAlberoVisitsByAutoGeneratedUsers() {
        log.debug("REST request to get all AlberoVisits");
        return alberoVisitCustomService.findAllByAutogeneratedUser();
    }
    ...
    public ResponseEntity<AlberoVisitDTO> getAlberoVisit(@PathVariable Long id) {
        ...
        Optional<AlberoVisitDTO> alberoVisitDTO = alberoVisitCustomService.findOne(id);
    ...
    public ResponseEntity<Void> deleteAlberoVisit(@PathVariable Long id) {
        ...
        alberoVisitCustomService.delete(id);
