...
import cloud.nino.nino.service.custom.EventAndLocationCustomService;
...
public class EventAndLocationCustomResource {

    private final Logger log = LoggerFactory.getLogger(EventAndLocationCustomResource.class);

    private static final String ENTITY_NAME = "eventAndLocation";

    private final EventAndLocationCustomService eventAndLocationCustomService;

    public EventAndLocationCustomResource(EventAndLocationCustomService eventAndLocationCustomService) {
        this.eventAndLocationCustomService = eventAndLocationCustomService;
    }
    ...
    public ResponseEntity<EventAndLocationDTO> createEventAndLocation(@RequestBody EventAndLocationDTO eventAndLocationDTO) throws URISyntaxException {
        ...
        EventAndLocationDTO result = eventAndLocationCustomService.save(eventAndLocationDTO);
    ...
    public List<EventAndLocationDTO> getAllEventAndLocations(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        ...
        return eventAndLocationCustomService.findAll();
    }

    /**
     * GET  /event-and-locations/by-current-user : get all the eventAndLocations.
     *
     * @param eagerload flag to eager load entities from relationships (This is applicable for many-to-many)
     * @return the ResponseEntity with status 200 (OK) and the list of eventAndLocations in body
     */
    @GetMapping("/event-and-locations/by-current-user")
    @Timed
    public List<EventAndLocationDTO> getAllEventAndLocationsByCurrentUser(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all EventAndLocations");
        return eventAndLocationCustomService.findAllByCurrentUser();
    }
    ...
    public ResponseEntity<EventAndLocationDTO> getEventAndLocation(@PathVariable Long id) {
        ...
        Optional<EventAndLocationDTO> eventAndLocationDTO = eventAndLocationCustomService.findOne(id);
    ...
    public ResponseEntity<Void> deleteEventAndLocation(@PathVariable Long id) {
        ...
        eventAndLocationCustomService.delete(id);
